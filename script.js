...
class waveNoise{
    ...
    addWaves(reuiredWaves){
        for(let i = 0; i < reuiredWaves; i++){
...
class Animation {
    ...
    createControls(){
        for (let i = 0; i < this.controlsNum + cfg.curvesNum; ++i ){
            ...
            control.addWaves(cfg.waveToBlend);
    ...
    updateCurves() {
        ...
        let _controlX2 = c[1].getWave() * this.size.w;
        let _controlY1 = c[2].getWave() * this.size.h;
        ...
        for(let i = 0; i < cfg.curvesNum; ++i){
            ...
            let curveParam = {
                startX : 0,
                startY : this.getYPlacamentType(this.type4Start, i), 
                controlX1 : _controlX1,
                controlY1 : _controlY1, 
                controlX2 : _controlX2,
                controlY2 : _controlY2 ,
                endX      : this.size.w,
                endY      : this.getYPlacamentType(this.type4End, i),
                alpha     : _controlY2,
            }
            this.drawCurve(curveParam)
        }
    }
    drawCurve({startX, startY, controlX1, controlX2,controlY1,controlY2,endX,endY,alpha}){
    ...
    updateFrameCounter(){
        this.framesCounter = (this.framesCounter + 1) % cfg.framesToMove
